...
import { Award, Package, Calendar, Building, Mail, User as UserIcon, Trash2, Edit, X, Camera, MapPin, Clock, Shield, AlertTriangle, CheckCircle2, XCircle, Upload, Users, ChevronRight } from 'lucide-react';
...
const Profile = () => {
  ...
  const handleDeleteItem = async (id: string) => {
    if (window.confirm('Delete this item? Points will be deducted if it was borrowed.')) {
      await deleteItem(id);
    }
  };
  ...
  const handleDeleteEvent = async (id: string) => {
    if (window.confirm('Delete this event? Your 30-point organizer bonus will be revoked.')) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-24 md:pb-12 px-4">
      {/* Profile Header */}
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
          <div className="relative group shrink-0">
            <div className="w-32 h-32 bg-accent/20 rounded-[32px] flex items-center justify-center border-4 border-white shadow-xl overflow-hidden">
              {user.imageUrl ? <img src={user.imageUrl} alt={user.name} className="w-full h-full object-cover" /> : <UserIcon className="w-16 h-16 text-primary" />}
            </div>
            <button onClick={() => fileInputRef.current?.click()} className="absolute bottom-0 right-0 p-2 bg-primary text-surface rounded-xl shadow-lg border-2 border-white active:scale-95"><Camera className="w-4 h-4" /></button>
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-gray-500 font-medium text-sm">
              ...
              <div className="flex items-center gap-1.5 font-black text-primary bg-accent/20 px-3 py-0.5 rounded-lg uppercase text-[10px] tracking-widest"><span>{user.points || 0} Pts</span></div>
            </div>
            <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-4">
              {user.badges.map(badge => <span key={badge} className="bg-amber-50 text-amber-600 border border-amber-100 px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2">{badge}</span>)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <h3 className="text-2xl font-black text-gray-900 flex items-center gap-2 px-2 tracking-tight"><Package className="w-6 h-6 text-secondary" /> My Posted Items</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {userItems.length === 0 ? (
            <div className="col-span-full bg-white rounded-3xl p-12 text-center border-2 border-dashed border-gray-100"><p className="text-gray-500">No items posted yet.</p></div>
          ) : (
            userItems.map(item => (
              <div key={item.id} className="bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-sm flex flex-col group hover:shadow-xl transition-all">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <h4 className="font-bold text-gray-900 text-lg mb-4 line-clamp-1">{item.title}</h4>
                  <button onClick={() => handleToggleAvailability(item.id, item.available)} className={cn("w-full mt-auto py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all", item.available ? "bg-amber-50 text-amber-600 hover:bg-amber-100" : "bg-primary text-surface hover:bg-primary/90")}>
      ...
      {/* Organized Events */}
      <div className="space-y-6">
        <h3 className="text-2xl font-black text-gray-900 flex items-center gap-2 px-2 tracking-tight"><Calendar className="w-6 h-6 text-primary" /> My Organized Events</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {myEvents.map(event => (
            <div key={event.id} className="relative overflow-hidden bg-white rounded-[32px] border border-gray-100 shadow-sm min-h-[220px] flex flex-col group hover:shadow-xl transition-all">
              {event.imageUrl && <><img src={event.imageUrl} className="absolute inset-0 w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-gray-900/80 to-transparent"></div></>}
              <div className={cn("relative z-10 p-6 flex-1 flex flex-col", event.imageUrl ? "text-white" : "text-gray-900")}>
                ...
                <h4 className="font-black text-lg leading-tight mb-2">{event.title}</h4>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Joined Events */}
      <div className="space-y-6">
        <h3 className="text-2xl font-black text-gray-900 flex items-center gap-2 px-2 tracking-tight"><Users className="w-6 h-6 text-secondary" /> Joined Events</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {joinedEvents.length === 0 ? <p className="px-4 text-gray-400 font-medium">No events joined yet.</p> : joinedEvents.map(event => (
            <div key={event.id} className="relative overflow-hidden bg-white rounded-[32px] border border-gray-100 shadow-sm min-h-[220px] flex flex-col">
              {event.imageUrl && <><img src={event.imageUrl} className="absolute inset-0 w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-gray-900/80 to-transparent"></div></>}
              <div className={cn("relative z-10 p-6 flex-1 flex flex-col", event.imageUrl ? "text-white" : "text-gray-900")}>
                <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-2.5 flex flex-col items-center justify-center w-[55px] shadow-lg border border-accent/20 mb-4">
                ...
                <h4 className="font-black text-base mb-2">{event.title}</h4>
                <div className="mt-auto flex items-center justify-between">
                   <span className="text-[10px] font-black uppercase tracking-tighter opacity-60">By {event.creatorName}</span>
                   <span className="bg-accent text-primary px-3 py-1 rounded-full text-[9px] font-black uppercase">Going</span>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* Cleanup Reports */}
      <div className="space-y-6">
        <h3 className="text-2xl font-black text-gray-900 flex items-center gap-2 px-2 tracking-tight"><Shield className="w-6 h-6 text-primary" /> My Cleanup Reports</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {myCleanupPosts.map(place => (
            <div key={place.id} className="bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-sm group hover:shadow-xl transition-all">
              <div className="relative h-40 bg-gray-50">
                ...
                <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => deleteCleanupPlace(place.id)} className="p-2 bg-white/95 text-red-500 rounded-xl shadow-md"><Trash2 className="w-4 h-4" /></button></div>
                <div className="absolute top-4 left-4"><span className="bg-surface/90 backdrop-blur-sm text-primary px-3 py-1 rounded-full text-[10px] font-black uppercase">{place.status}</span></div>
              </div>
              <div className="p-6">
                <h4 className="font-bold text-gray-900 mb-4 line-clamp-1">{place.title}</h4>
                {user.role === 'admin' ? (
                  <button onClick={() => updateCleanupStatus(place.id, place.status === 'pending' ? 'cleaning' : 'completed')} disabled={place.status === 'completed'} className={cn("w-full py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all", place.status === 'completed' ? "bg-gray-100 text-gray-400" : "bg-secondary text-surface hover:bg-secondary/90 shadow-lg")}>
                    {place.status === 'pending' ? 'Start Cleaning' : place.status === 'cleaning' ? 'Mark Done' : 'Verified'}
                  </button>
                ) : (
                  <div className="bg-gray-50 py-3 rounded-xl text-center border border-gray-100"><span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Admin Only Control</span></div>
                )}
              </div>
            </div>
          ))}
      ...
      {editingItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/30 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[32px] p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black">Edit Item</h2><button onClick={() => setEditingItem(null)} className="p-2 hover:bg-gray-100 rounded-full"><X className="w-6 h-6 text-gray-400" /></button></div>
            <form onSubmit={handleUpdateItem} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-primary text-surface py-4 rounded-2xl font-black uppercase tracking-widest text-sm shadow-xl">Save Changes</button>
            </form>
          </div>
        </div>
      )}
      ...
      {editingEvent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/30 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[32px] p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black">Edit Event</h2><button onClick={() => setEditingEvent(null)} className="p-2 hover:bg-gray-100 rounded-full"><X className="w-6 h-6 text-gray-400" /></button></div>
            <form onSubmit={handleUpdateEvent} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-primary text-surface py-4 rounded-2xl font-black uppercase tracking-widest text-sm shadow-xl">Update Event</button>
            </form>
          </div>
        </div>
      )}
      ...
      {isEditingProfile && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/30 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[32px] p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black text-gray-900">Update Profile</h2><button onClick={() => setIsEditingProfile(false)}><X className="w-6 h-6 text-gray-400" /></button></div>
